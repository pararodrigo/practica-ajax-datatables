...
-- Tiempo de generación: 06-04-2015 a las 11:17:05
...
CREATE TABLE `tarifas_articulos` (
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `datos` AS select `d`.`id_doctor` AS `id_doctor`,`d`.`nombre` AS `nombre_doctor`,`d`.`numcolegiado` AS `numcolegiado`,group_concat(`c`.`nombre` separator '</li><li>') AS `clinicas` from ((`doctores` `d` join `clinica_doctor` `cd`) join `clinicas` `c`) where ((`cd`.`id_doctor` = `d`.`id_doctor`) and (`cd`.`id_clinica` = `c`.`id_clinica`)) group by `d`.`nombre`;
...
--
-- Indices de la tabla `articulos_materiales`
--
ALTER TABLE `articulos_materiales`
 ADD PRIMARY KEY (`articulos_id_articulo`,`materiales_id_material`), ADD KEY `fk_articulos_materiales_articulos` (`articulos_id_articulo`), ADD KEY `fk_articulos_materiales_materiales` (`materiales_id_material`);

--
-- Indices de la tabla `clinicas`
--
ALTER TABLE `clinicas`
 ADD PRIMARY KEY (`id_clinica`), ADD KEY `fk_clinicas_tarifas1` (`id_tarifa`);

--
-- Indices de la tabla `clinica_doctor`
--
ALTER TABLE `clinica_doctor`
 ADD PRIMARY KEY (`id_doctor`,`id_clinica`), ADD KEY `fk_clinica_doctor_Doctores` (`id_doctor`), ADD KEY `fk_clinica_doctor_clinicas1` (`id_clinica`);
...
--
-- Indices de la tabla `tarifa_articulos`
--
ALTER TABLE `tarifa_articulos`
 ADD PRIMARY KEY (`tarifas_id_tarifa`,`articulos_id_articulo`), ADD KEY `fk_tarifa_articulos_tarifas` (`tarifas_id_tarifa`), ADD KEY `fk_tarifa_articulos_articulos` (`articulos_id_articulo`);
